...
function computeWordCounts(state: any): DecorationSet {
    const builder = new RangeSetBuilder<Decoration>();
    const doc = state.doc;
    const lines = doc.lines;

    // Active headers for H1, H2, H3 (indices 1, 2, 3)
    let activeHeaders: ({ from: number, to: number, linePos: number } | null)[] = [null, null, null, null];
    let decorationsBuffer: { pos: number, widget: Decoration }[] = [];
